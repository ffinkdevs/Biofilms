...
// Pure-Odin port of Julia 1.12's Base.Math exp(::Float64)
// (base/special/exp.jl): table-driven argument reduction with a
// degree-4 minimax kernel. This is NOT libm/openlibm exp — Julia 1.12
// computes it inline, and it differs from libm by 1 ulp on ~10% of
// inputs. Verified bit-exact vs Julia 1.12.6 over dense sweeps.
//
// Every muladd below is a TRUE fused multiply-add (single rounding),
// matching Julia's Base.muladd → llvm.fma. Implemented via C fma()
// (correctly rounded per IEEE, identical to hardware vfmadd). Do NOT
// replace with a*b+c (double rounding breaks parity), and beware that
// -o:speed codegen must not reassociate these expressions — the sweep
// test pins both -o:none and -o:speed.
...
foreign libc {
	@(link_name="fma")
	c_fma :: proc(x, y, z: f64) -> f64 ---
}

// Single-rounding fused multiply-add (Julia Base.muladd semantics).
jfma :: proc(x, y, z: f64) -> f64 {
	return c_fma(x, y, z)
}

J_MAGIC_ROUND :: 6.755399441055744e15
J_LOGBO256INV :: 369.3299304675746
J_LOGBO256U :: -0.002707606173999011
J_LOGBO256L :: -6.327543041662719e-14
J_SUBNORM_EXP :: 708.3964185322641
J_MAX_EXP :: 709.7827128933841
J_MIN_EXP :: -745.1332191019412
J_TWO_POW_M53 := transmute(f64)u64(0x3CA0000000000000) // 2^-53

J_TABLE := [256]u64{
	0x0000000000000000, 0xaac00b1afa5abcbe, 0x9b60163da9fb3335, 0xab502168143b0280, 0xadc02c9a3e778060,
...
// Degree-4 minimax kernel, Horner with FMA (Julia evalpoly semantics).
@(private)
jexp_kernel :: proc(r: f64) -> f64 {
	q := jfma(r, 0.04166666857598777, 0.1666666857598779)
	q = jfma(r, q, 0.4999999999999997)
	q = jfma(r, q, 0.9999999999999912)
	return r * q
}

// Julia 1.12 Base.Math exp(::Float64). Bit-exact port of exp_impl.
jexp_f64 :: proc(x: f64) -> f64 {
	n_float := jfma(x, J_LOGBO256INV, J_MAGIC_ROUND)
	n := i32(transmute(u64)n_float)
	n_float -= J_MAGIC_ROUND
	r := jfma(n_float, J_LOGBO256U, x)
	r = jfma(n_float, J_LOGBO256L, r)
	k := n >> 8
	j := u32(n) & 255
	jt := J_TABLE[j]
	ju := transmute(f64)(u64(0x3FF0000000000000) | (jt & u64(0x000FFFFFFFFFFFFF)))
	jl := transmute(f64)(u64(0x3C00000000000000) | (jt >> 8))
	small_part := jfma(ju, jexp_kernel(r), jl) + ju

	if !(abs(x) <= J_SUBNORM_EXP) {
		if math.is_nan(x) { return x }
		if x >= J_MAX_EXP { return math.INF_F64 }
		if x <= J_MIN_EXP { return 0.0 }
		if k <= -53 {
			twopk := (u64(i64(k)) + u64(53)) << 52
			return transmute(f64)(twopk + transmute(u64)small_part) * J_TWO_POW_M53
		}
	}
	twopk := u64(i64(k)) << 52
	return transmute(f64)(twopk + transmute(u64)small_part)
}
